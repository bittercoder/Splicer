...
namespace Splicer.Utils
{
    ...
    public sealed class FilterGraphTools
    {
        ...
        public static readonly Guid CLSID_AudioCompressorCategory = new Guid("33d9a761-90c8-11d0-bd43-00a0c911ce86");
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public static List<PinQueryInfo> EnumeratePins(IBaseFilter filter)
        {
            if (filter == null) throw new ArgumentNullException("filter");
            int hr = 0;

            List<PinQueryInfo> pinsInfo = new List<PinQueryInfo>();
        ...
        public static IPin FindPinForMajorType(IBaseFilter filter, PinDirection direction, Guid majorType)
        {
            ...
            try
            {
                ...
                while (fetched > 0)
                {
                    hr = pinsEnum.Next(1, pins, out fetched);
                    DsError.ThrowExceptionForHR(hr);
                    ...
                    if (fetched > 0)
                    {
                        ...
                        try
                        {
                            ...
                            while (fetched2 > 0)
                            {
                                hr = mediaTypesEnum.Next(1, mediaTypes, out fetched2);
                                DsError.ThrowExceptionForHR(hr);
